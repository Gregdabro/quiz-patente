...
- **Работа с Hooks:** Всегда использовать стандартную ES6-деструктуризацию (`const [state, setState] = useState(...)`). Строго избегать антипаттернов с ручным доступом по индексам. Использование `const` и стрелочных функций в React компонентах безопасно — `@vitejs/plugin-legacy` выполняет транспиляцию в ES5 для Safari 12.
