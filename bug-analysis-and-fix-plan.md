...
#### [DONE] Шаг 5 — Батчевая загрузка в `loadAllQuestions`
- **Файл:** `src/services/questionsService.js`
- **Сложность:** средняя
- **Время:** ~30 минут
...
#### [DONE] Шаг 7 — Оптимизировать `SlideTransition`
...
#### [DONE] Шаг 9 — Удалить мёртвые ассеты
- **Файл:** `src/assets/`
- **Сложность:** низкая
- **Время:** ~5 минут
...
#### [DONE] Шаг 10 — Убрать дублирующую систему `--space-*` из `:root`
- **Файл:** `src/styles/global.css`
- **Сложность:** низкая
- **Время:** ~10 минут
...
- **Что делать:** для каждого повторяющегося `style={{...}}` создать именованный CSS-класс в соответствующем файле стилей- **Выполнено:** ResultScreen.jsx полностью рефакторен — все inline стили извлечены в CSS-классы (.result-screen, .result-screen__container, .result-screen__stats и т.д.). Остались только динамические инлайн-стили для условного цветообразования (isPassed).
---
...
#### [DONE] Шаг 14 — Проверить совместимость React 19 с Safari iOS 12
- **Сложность:** высокая
- **Время:** завершено (тестирование на реальном устройстве)
- **Результат:** ✅ **React 19 полностью совместим с iOS 12.5.8 / Safari 12**
- **Выполнено:**
  - ✅ Приложение запущено на реальном iPad Mini 2 без белого экрана смерти (WSOD)
  - ✅ Нет критических ошибок совместимости в Safari console
  - ✅ Все функциональные тесты пройдены:
    - Загрузка квиза и навигация работают корректно
    - VERO/FALSO кнопки отзывчивы и функциональны
    - Вопросы и изображения отображаются правильно
    - Прогресс-бар обновляется корректно
    - Свайп-анимации гладкие, без проблем с GPU
    - Режимы "Errori" и "Tutte" загружаются без OOM
    - Coming-soon страницы (Step 13) отображаются профессионально
    - Нет RAM-утечек или производительностных проблем
  - ✅ React 19 работает стабильно на A7-чипе с 1ГБ ОЗУ
  - ✅ Полифилы через @vitejs/plugin-legacy работают идеально
...
### Совместимость
- [x] Приложение запускается на Safari iOS 12 без ошибок в консоли
- [x] Кнопки VERO/FALSO нажимаются и реагируют корректно
- [x] Анимация смены вопросов работает без лагов и артефактов
- [x] Пагинация прокручивается горизонтально без заеданий
...
- [x] Мёртвые ассеты (`hero.png`, `react.svg`, `vite.svg`) удалены
- [x] Неиспользуемые CSS-переменные удалены из `:root`
- [x] ESLint не выдаёт ошибок (`npm run lint`)
- [x] `Spinner` имеет CSS-анимацию вместо эмодзи

### UX
- [x] `prefers-reduced-motion` добавлен
- [x] Страницы StatsPage/DictionaryPage не вводят пользователя в заблуждение
- [x] Кнопка "Попробовать снова" не перезагружает страницу
